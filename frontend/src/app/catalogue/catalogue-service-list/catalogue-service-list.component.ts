import { BuyServiceComponent } from './buy-dialog/buy-dialog.component';
...
@Component({
  selector: 'catalogue-service-list',
  templateUrl: './catalogue-service-list.component.html',
  styleUrls: ['./catalogue-service-list.component.css']
})
export class CatalogueServiceListComponent implements OnInit, OnDestroy{
  ...
  ngOnInit(){
    this.httpClient.get(environment.endpointURL + 'item/getSer/', {
    }).subscribe(data => {
  ...
  openDialog(itemId): void {
    this.itemId = itemId
    const dialogRef = this.dialog.open(BuyServiceComponent, {
      width: '250px',
      data: this.itemId
    });
  }
